...
public class ExpenseReportService {
    ...
    public ExpenseReportInfoDTO getExpenseReportItemInfo(Integer id) {
        ...
        long tripLengthInDays = java.time.Duration.between(trip.getDatetimeFrom().toInstant(), trip.getDatetimeTo().toInstant()).toDays() + 1;
        ...
        List<ExpenseCategoryDTO> expenseCategoryDTOs = expenseReport.getExpenseReportItems().stream()
                .collect(Collectors.groupingBy(item -> item.getExpenseSubcategory().getExpenseCategory()))
                .entrySet().stream()
                .map(entry -> {
                    ExpenseCategory expenseCategory = entry.getKey();
                    List<ExpenseReportItemInfoDTO> expenseReportItemInfoDTOs = entry.getValue().stream()
                            .map(expenseReportItem -> new ExpenseReportItemInfoDTO(
                                    expenseReportItem.getId(),
                                    expenseReportItem.getReceipt().getId(),
                                    new ExpenseSubcategoryDTO(expenseReportItem.getExpenseSubcategory()),
        ...
        return new ExpenseReportInfoDTO(
                new UserDetailsDTO(user.getId(), user.getEmail(), user.getFirstName(), user.getLastName(), user.getIban()),
                new TripDTO(
                        trip.getCoordinatesLat(), trip.getCoordinatesLon(), trip.getAddress(),
                        trip.getCity(), trip.getCountry().getCode(), trip.getDatetimeFrom(), trip.getDatetimeTo(), trip.getReason()
                ),
                expenseCategoryDTOs,
                dailyWageDTO,
                eurExpenseSum
